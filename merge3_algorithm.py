...
def read_file(fp):
    ...
    # print(f"[INFO] {fp} dosyasýndan {len(lines)} satýr okundu.")
    return lines

def write_file(fp, lines):
    with open(fp, 'w') as f: f.writelines(lines)
    print(f"\n[INFO] {fp} dosyasýna {len(lines)} satýr yazýldý.")
